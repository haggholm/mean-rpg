...
module.exports = app.controller('RollBreakdownCtrl', function($scope){
  ...
  $scope.oddsVsAdvantageData = formatData(simdata.stats);
  $scope.oddsVsAdvantageDataNoDraw = formatData(simdata.stats, true);
  ...
  function formatPercentage(pct) {
    if (isNaN(pct)) {
      ...
      pct = Number(pct);
      if (pct === 100) {
        return (precision > 3 ? pct.toFixed(precision-3) : pct) + '%';
      } else if (pct < 10) {
        return pct.toFixed(precision-1) + '%';
      } else {
        return pct.toFixed(precision-2) + '%';
      }
    }
  }
